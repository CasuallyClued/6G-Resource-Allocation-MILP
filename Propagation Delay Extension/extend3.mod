...
var RSdt{d in N, t in T} >= 0;	# Energy  be charged in the battery from the surplus renewable energy at FDC d at time t
...
var FDCBserved{d in N} = sum{s in S, t in T} VoDFBdst[d,s,t] * OLTdt[d,t]; # Volume of fog served VoD traffic
var FDCSserved{d in N} = sum{s in S, t in T} VoDFSdst[d,s,t] * OLTdt[d,t]; # Volume of fog served VoD traffic
var FDCEserved{d in N} = sum{s in S, t in T} VoDFEdst[d,s,t] * OLTdt[d,t]; # Volume of fog served VoD traffic
...
var CDCS{t in T} = sum{c in CDC, d in N} VoDCcdt[c,d,t];				# Volume of cloud served traffic at time t
var FDCB{t in T} = sum{d in N, s in S} VoDFBdst[d,s,t] * OLTdt[d,t];	# Volume of brown FDC served traffic at time t
var FDCS{t in T} = sum{d in N, s in S} VoDFSdst[d,s,t] * OLTdt[d,t];	# Volume of solar FDC served traffic at time t
var FDCE{t in T} = sum{d in N, s in S} VoDFEdst[d,s,t] * OLTdt[d,t];	# Volume of ESD FDC served traffic at time t
